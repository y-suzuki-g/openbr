...
class CvCascadeTransform : public MetaTransform
{
    ...
    // Training parameters
    Q_PROPERTY(QString vec READ get_vec WRITE set_vec RESET reset_vec STORED false)
    ...
    BR_PROPERTY(QString, vec, "vector.vec")
    ...
    void train(const TemplateList& data)
    {
        ...
        for (int i = 0; i < files.length(); i++) {
            ...
            if (f.contains("training-set")) {
                ...
                // Positive samples for crop/rescale    
                    ...
                    posStream << f.path() << QDir::separator() << f.fileName() << " " << 1 << " " << "0 0 24 24" << endl;
        ...
        params.data = Globals->sdkPath + "/share/openbr/models/openbrcascades/" + model;
...
class KeyToRectTransform : public UntrainableMetadataTransform
{
    ...
    void projectMetadata(const File &src, File &dst) const
    {
        dst = src;
        if (!key.isEmpty()) {
            dst.appendRect(QRect(src.get<int>(key+"_X"),
                                 src.get<int>(key+"_Y"),
